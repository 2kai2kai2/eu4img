...
def fileLoadGuilds() -> List[GuildSave]:
	...
	try:
		...
		if len(jsonLoad) == 0:
			return []
	for gld in jsonLoad:
		guilds.append(GuildSave(gld, jsonLoad[gld]["prefix"], jsonLoad[gld]["adminRank"], jsonLoad[gld]["announcements"], jsonLoad[gld]["defaultBan"]))
	return guilds
...
def addGuild(guild: Union[discord.Guild, int], prefix: Optional[str] = "$", admin: Optional[str] = None, conn: Optional[psycopg2.extensions.connection] = None):
	...
	# SQL
		...
		try:
			...
			cur.execute("CREATE TABLE Guilds (name varchar, prefix varchar, admin varchar, announcements varchar, defaultBan varchar[])")
		else:
			if cur.fetchone() is not None: # Somehow we ended up joining a guild we already have saved.
				return # Do nothing.
			else:
				cur.execute("INSERT INTO Guilds (name, prefix, admin, announcements, defaultBan) VALUES (%s, %s, %s, %s, %s)", [guildName, prefix, admin, None, []])	
		finally:
			cur.close()
	...
	# SQL
		...
		try:
			...
			if current is None:
				...
				cur.execute("INSERT INTO Guilds (name, prefix, admin, announcements, defaultBan) VALUES (%s, %s, %s, %s, %s)", [guildName, prefix, current[2], current[3], current[4]])	
		finally:
			cur.close()
	...
	# SQL
		...
		try:
			...
			if current is None:
				...
				cur.execute("DELETE FROM Guilds WHERE name=%s", [guildName])
				cur.execute("INSERT INTO Guilds (name, prefix, admin, announcements, defaultBan) VALUES (%s, %s, %s, %s, %s)", [guildName, current[1], admin, current[3], current[4]])	
		finally:
			cur.close()
	...
	# SQL
		...
		try:
			...
			if current is None:
				...
				cur.execute("DELETE FROM Guilds WHERE name=%s", [guildName])
				cur.execute("INSERT INTO Guilds (name, prefix, admin, announcements, defaultBan) VALUES (%s, %s, %s, %s, %s)", [guildName, current[1], current[2], announcements, current[4]])	
		finally:
			cur.close()
	...
	# SQL
		...
		try:
			...
			if current is None:
				...
				cur.execute("DELETE FROM Guilds WHERE name=%s", [guildName])
				cur.execute("INSERT INTO Guilds (name, prefix, admin, announcements, defaultBan) VALUES (%s, %s, %s, %s, %s)", [guildName, current[1], current[2], current[3], newBanList])	
		finally:
			cur.close()
